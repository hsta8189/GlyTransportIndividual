function [t,z] = GlyT1_halt_ode(z0, k, kinv, c, tau, tspan, fix_GlyE, opts)
...
%Integration step
[t,z] = ode15s(@(t,z) odefcn(t,z, k, kinv, Gly_e0), tspan, z0,opts);



%% set up ODE

function dzdt = odefcn(t,z, k, kinv, Gly_e0)
dzdt = zeros(14,1);
...
Cl_e = z(12);
Gly_i = z(13);
Gly_e = heaviside( tspan(end) / 2 - t) * z(14) +  heaviside(  t - tspan(end) / 2 ) * Gly_e0; 
z(14) = Gly_e;
...
dzdt(14) = heaviside(  tspan(end) / 2 - t) * J_Chi_e; % extracellular Glycine
